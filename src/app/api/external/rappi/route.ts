...
// ─── Tipos para el payload de Rappi ───
...
export async function GET() {
  return NextResponse.json({ status: 'ok', platform: 'rappi' })
}

// ─── Recibir orden desde Rappi ───

export async function POST(req: Request) {
  try {
    // Validar webhook secret
    const secret = req.headers.get('X-Webhook-Secret')
    const expectedSecret = process.env.RAPPI_WEBHOOK_SECRET

    if (!expectedSecret) {
      console.error('[Rappi Webhook] RAPPI_WEBHOOK_SECRET no configurado')
    ...
    const body: RappiOrderPayload = await req.json()
    ...
    if (body.externalId) notesParts.push(`Rappi ID: ${body.externalId}`)
    ...
    console.log(`[Rappi Webhook] Orden #${orderNumber} creada (ext: ${body.externalId})`)
    ...
    console.error('[Rappi Webhook] Error procesando orden:', e)
    return NextResponse.json(
      { error: 'Error procesando orden de Rappi', detail: String(e) },
      { status: 500 }
    )
  }
}
